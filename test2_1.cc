...
void *producer (void *arg) {
	...
	cout << " producer: opened HeapDBFile " << rel->path () << endl;
...
void *consumer (void *arg) {
	...
	if (t->write) {
		...
		heapdbfile.Create (outfile, heap, NULL);
	}
	...
	if (t->write) {
		...
		cerr << " consumer: recs removed written out as heap HeapDBFile at " << outfile << endl;
		heapdbfile.Close ();
	}
